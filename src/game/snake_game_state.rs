...
impl SnakeGameState {
    pub fn new() -> Self {
        ...
        SnakeGameState {
            ...
            cpu_player,
            map: Map::new(),
            ai: Ai::with_behavior(Box::new(super::ai::ChasePlayerAi {})),
            cpu_ai: Ai::with_behavior(Box::new(super::ai::ChaseFruitAi {})),
    ...
    fn gui_render(& mut self, ctx: &mut BTerm) {
        ...
        ctx.print_color_centered(1, ENEMY_COLOR, BACKGROUND_COLOR, &format!("Purple score is: {}", self.enemy.get_length()));
    ...
    fn move_cpu_player(& mut self) {
        ...
        if self.cpu_player.collide(new_pos) {
            self.respawn_cpu_player();
        } else if self.player.collide(new_pos) {
